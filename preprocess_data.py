...
class BrainPipeline(object):
    ...
    def read_scans(self):
        ...
        t1 = glob(self.path + '/*_T1.*/*.mha')
        t1c = glob(self.path + '/*_T1c.*/*.mha')
        t2 = glob(self.path + '/*_T2.*/*.mha')
        gt = glob(self.path + '/*OT.*/*.mha')
        t1s = glob(self.path + '/**/*T1*.mha')
       
        scans = [flair[0], t1[0], t1c[0], t2[0], gt[0]] # directories to each image (5 total)
        if self.n4itk_apply:
...
def save_labels(labels):
    ...
    for label_idx in range(len(labels)):
        slices = io.imread(labels[label_idx], plugin='simpleitk').astype(float)
        for slice_idx in range(len(slices)):
            strip = slices[slice_idx].reshape(240, 240)
            if np.max(strip) != 0: # set values < 1
                strip /= np.max(strip)
            if np.min(strip) <= -1: # set values > -1
                strip /= abs(np.min(strip))
            io.imsave('Labels/{}_{}L.png'.format(label_idx, slice_idx), strip)


if __name__ == '__main__':
    current_path = os.getcwd()
    label_path = os.path.join(current_path,'Labels') 
    train_path = os.path.join(current_path,'Training_PNG') 
    norm_path = os.path.join(current_path,'Norm_PNG')
    n4_path = os.path.join(current_path,'n4_PNG')
    ...
    if os.path.isdir(n4_path):
        shutil.rmtree(n4_path)
    os.makedirs(n4_path)


    labels = glob('Original_Data/HGG/**/**/**.mha')
    save_labels(labels)

    patients = glob('Original_Data/HGG/**')
